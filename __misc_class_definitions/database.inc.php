				...
				case 'svc_isValidLoginData':
					...
					if($mysqli->error){
						...
						if(password_verify($oUser->getReqParamByKey('PWDHASH'), self::$result_ARRAY[0][4])){
							...
							self::$query = 'UPDATE `users` SET `users`.`LASTLOGIN`="'.$ts.'",`users`.`LOGIN_CNT`="'.self::$result_ARRAY[0][7].'",`users`.`DATEMODIFIED`="'.$ts.'" WHERE `users`.`USERNAME`="'.self::$result_ARRAY[0][0].'" AND `users`.`USERNAME_CRC32`="'.crc32(self::$result_ARRAY[0][0]).'" AND `USERID_SOURCE`="'.md5(self::$result_ARRAY[0][0]).'" AND `USERID`="'.crc32(md5(self::$result_ARRAY[0][0])).'" LIMIT 1;';
